...
public class BuildAMatrixWithConditions {
	...
	// https://leetcode.com/problems/build-a-matrix-with-conditions/submissions/1327943983/
	public static void main(String[] args) {
		BuildAMatrixWithConditions test = new BuildAMatrixWithConditions();
		System.out.println(test.buildMatrix(0, new int[][] {
			{ 1, 2 },
			{ 3, 2 }
		}, new int[][] {
			{ 2, 1 },
	...
	public int[][] buildMatrix(int k, int[][] rowConditions, int[][] colConditions) {
		...
		for (int i = 1; i < rowGraph.length; i++) {
			if (!this.sort(rowGraph, i, visited, queue)) {
				return new int[0][0];
			}
		}
		...
		for (int i = 1; i < colGraph.length; i++) {
			if (!this.sort(colGraph, i, visited, queue)) {
				return new int[0][0];
			}
		}
	...
	private boolean sort(List<Integer>[] graph, int node, int[] visited, Deque<Integer> queue) {
		if (visited[node] == 2) {
			return false;
		}
		if (visited[node] == 0) {
			...
			for (int child : graph[node]) {
				if (!this.sort(graph, child, visited, queue)) {
					return false;
				}
			}
			visited[node] = 1;
